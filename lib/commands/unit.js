...
function set_raw_flag(flags){
  ...
  for(f in command_flags){
    var curFlag = command_flags[f];
    if(curFlag.internal_type == "print")
...
function print_burst(burst, print_raw, sub_point) {
  ...
  if (print_raw) {
    ...
    return burst["name"] + " - (" + endLevel["bc cost"] + "BC/" + numHits + "hits/" + dc + "DC) " + burst["desc"];
  }
}
  ...
  if (print_raw) {
    ...
    for (s in sp) {
      ...
      if (curSkill["id"] == id) {
        ...
        msg += "**[**" + curSkill["bp"] + " SP**]** | (" + getSPCategory(sp[s]["category"]) + ") - " + curSkill["desc"] + " (" + curSkill["id"] + ")\n";
  ...
  if (!isNaN(id)) {//if given an id, get the unit first then try printing again
    ...
    try {
      ...
      if (flags.raw) { //print raw stats
        if (flags.p_sp_skill == "" && flags.p_hitcount == "") {
          if (flags.p_ls) {
            msg += "* LS:\n";
          ...
          if (flags.p_sp_skill != ""){//print sp info
