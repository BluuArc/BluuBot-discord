            ...
            return new Promise(function (fulfill, reject) {
                ...
                if (argv.flags.print_help || argv.flags.search_help || argv.flags.list_help) {
                    ...
                    if (argv.flags.print_help) { //print info about printing info
                        ...
                        internal_type = "list";
                        footer = "\nExample: `|bb unit --list_range --list_type unit --list_start 60660 --list_end 60670` prints all units whose ID starts with 60660 and the first unit above 60670\n";
                        footer += "Example: `|bb unit --list_range --list_start 1600` prints all units whose guide ID is 1600 or larger";
                    }
                ...
                //list commands
                if(argv.flags.list_range || argv.flags.list_count){
                    var list_query;
                    if (argv.flags.list_count) {
                        list_query = do_count_query(argv.flags.list_start, argv.flags.list_num_count, argv.flags.list_type)
                            .catch(function (err) {
                                console.log("Error in do_count_query:", err);
                                fulfill("Error: can't connect to database server");
                            });
                    } else {
                        list_query = do_range_query(argv.flags.list_start, argv.flags.list_end, argv.flags.list_type)
                            .catch(function(err) {
                            console.log("Error in do_range_query:", err);
                            fulfill("Error: can't connect to database server");
                        });
                    }
                    ...
                    var list_process = list_query.then(function(results){
                        if(results){
                            ...
                            return create_sectional_messages(formatted_results,900,5)
                                .then(function(msg_arr){
                                    if(context.useEmbed){
                                        var embed = {
                                            title: "Listing results",
                                            description: "Multiple units found. Please try the command again using one of the IDs below.",
                                            fields: []
                                        };
                                        if(msg_arr.length > 0){
                                        ...
                                        if (msg_arr.length > 0) {
                                            ...
                                            var u = 0;
                                            var max = (msg_arr.length < 100) ? msg_arr.length : 100;
                                            for (u = 0; u < max; ++u) {
                                                msg += msg_arr[u];
                                            }

                                            if (msg_arr.length > max) {
                                                var diff = msg_arr.length - u;
                ...
                if(context.useEmbed){
                    ...
                    if(argv.flags.print_hitcounts || argv.flags.print_sp || argv.flags.print_sp_skill.length > 0){
                        argv.flags.print_raw_effects = true;
                    }
                }
...
function search_unit(name,other_queries,context){
    ...
    print_debug_message(context, ["reached search unit with",name]);
...
function print_unit(unit_id, flags, context){
    ...
    function print_effects(effects) {
        ...
        //convert each effect into its own string
        // console.log(effects);
        if(effects["proc id"] == 55){
            return print_buffs(effects);
        }else{
            for (var param in effects) {
                if (param !== "passive id" && param !== "effect delay time(ms)\/frame") {
                    var tempText = effects[param];
    ...
    function print_embed(unit,flags){
        return new Promise(function(fulfill,reject){
            var embed = {
                ...
                title: `${unit.guide_id}: ${unit.name} (${unit.id})`,
                fields: []
            };
            ...
            if(flags.print_sp){
                    ...
                    for(var s = 0; s < unit.skills.length; ++s){
                        ...
                        for(var effect = 0; effect < curSkill.effects.length; ++effect){
                            if(curSkill.effects[effect] instanceof Array){
                                desc += print_array(curSkill.effects[effect]);
                            }else if(curSkill.effects[effect] instanceof Object){
                                for(var elem in curSkill.effects[effect]){
                                    desc += elem + ": " + print_effects(curSkill.effects[effect][elem]);
                                }
                            }else{
                ...
                print_evo(unit)
                    .then(function(result){
                        print_debug_message(context,["entered print evo with", result]);
                        ...
                        if (!flags.print_stats) {
                            embed.thumbnail = {
                                url: `${url}unit_ills_thum_${unit.id}.png`
                            };
                        }
                ...
                if(unit["leader skill"]){
                    try{
                        ...
                        if(flags.print_raw_effects){
                            ...
                            for(e in ls.effects){
                                extra_data += "```";
                                extra_data += print_effects(ls.effects[e]) + "\n";
                                extra_data += "```";
                            }
                        }
                        field_arr.push({
                ...
                if(unit["extra skill"]){
                    try{
                        ...
                        if (flags.print_raw_effects) {
                            ...
                            for (e in es.effects) {
                                extra_data += "```";
                                extra_data += print_effects(es.effects[e]) + "\n";
                                extra_data += "```";
                            }
                        }
                        field_arr.push({
    ...
    function print_regular(unit,flags){
        ...
        return new Promise(function(fulfill,reject){
            ...
            print_debug_message(context,["Entered print_regular with",flags]);
            try {
                var numHits = unit["damage frames"]["hits"];
                var msg = "";
                ...
                if (flags.print_raw_effects) { //print raw stats
                    print_debug_message(context,["entered raw_effects branch"]);
                    if (!flags.print_sp && !flags.print_evo && !flags.print_arena && !flags.print_stats && flags.print_sp_skill === "" && !flags.print_hitcounts) {
                        print_debug_message(context, ["entered individual print branch"]);
                        if (flags.print_ls) {
                            msg += "* LS: ";
                        ...
                        print_debug_message(context, ["entered sp/evo/arena/etc. branch"]);
                        if (flags.print_sp) {
                            if (unit["skills"] != undefined) {
                                msg += "SP Enhancements:\n"
    ...
    return new Promise(function(fulfill,reject){
        client.getUnits(results)
            .then(function(results){
                var msg = "Multiple units found. Please try the command again using one of the IDs below.\n";
                var formatted_results = [];
                for(var i = 0; i < results.length; ++i){
                    formatted_results.push(`${results[i].guide_id}: ${results[i].name} (${results[i].id})\n`);
                }
