                ...
                if(unit.bb){
                    try{
                        ...
                        if (flags.print_raw_effects && drop_checks_bb !== "None") {
                            ...
                            let burst_arr = common.createFieldsArray(`Brave Burst: ${bb.name}`, "", 
                                extra_data.split("\n")
                            ...
                            let local_field_arr = common.createFieldsArray(`Brave Burst: ${bb.name}`, "", [extra_data, `\n**Buff Translation:**\n${buff_translation.bb}`]);
                            for (let f of local_field_arr) {
                                field_arr.push(f);
                            }
                        }
                ...
                if (unit.sbb) {
                    try{
                        ...
                        if (flags.print_raw_effects && drop_checks_sbb !== "None") {
                            ...
                            let burst_arr = common.createFieldsArray("Super Brave Burst: " + sbb.name, "",
                                extra_data.split("\n")
                                    .filter(function (d) { return d.length > 0 })
                                    .map(function (d) { return `${d}\n`; }).concat([`\n**Buff Translation:**\n${buff_translation.sbb}`])
                            );
                            for (let b = 0; b < burst_arr.length; ++b) {
                                field_arr.push(burst_arr[b]);
                            }
                        }else{
                            let local_field_arr = common.createFieldsArray(`Super Brave Burst: ${sbb.name}`, "", [extra_data, `\n**Buff Translation:**\n${buff_translation.sbb}`]);
                            for (let f of local_field_arr) {
                                field_arr.push(f);
                            }
                        }
                ...
                if (unit.ubb) {
                    try{
                        ...
                        if (flags.print_raw_effects && drop_checks_ubb !== "None") {
                            ...
                            let burst_arr = common.createFieldsArray("Ultimate Brave Burst: " + ubb.name, "",
                                extra_data.split("\n")
                                    .filter(function (d) { return d.length > 0; })
                                    .map(function (d) { return `${d}\n`; }).concat([`\n**Buff Translation:**\n${buff_translation.ubb}`])
                            );
                            for (let b = 0; b < burst_arr.length; ++b) {
                                field_arr.push(burst_arr[b]);
                            }
                        }else{
                            let local_field_arr = common.createFieldsArray(`Ultimate Brave Burst: ${sbb.name}`, "", [extra_data, `\n**Buff Translation:**\n${buff_translation.ubb}`]);
                            for (let f of local_field_arr) {
                                field_arr.push(f);
                            }
                        }
