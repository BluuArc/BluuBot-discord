...
var command_flags = [
  ...
  {
    name: 'raw',
    desc: 'Print the raw data of a unit. Most p_commands require this; doesn\'t do anything on its own',
    type: 'boolean',
    internal_type: "print",
    default: false
  },
...
function print_help(header, internal_type, footer) {
  ...
  for (f in command_flags) {
    ...
    if (curFlag.internal_type == internal_type && !(curFlag.internal_type === 'print' && curFlag.name === 'raw')) {
      msg += "`--" + curFlag.name;
...
function print_hit_table(timing_array, distribution_array, nodiff){
  ...
  if(msg.length > 1700 && (nodiff === undefined || nodiff == true)){
...
function print_unit(id, flags) {
  ...
  if (!isNaN(id)) {//if given an id, get the unit first then try printing again
    ...
    var unit = id;
    if (isJapaneseText(JSON.stringify(unit))){ //translated unit
      return translate_unit(id).then(function(unit){
        return print_unit(unit, flags);
      })
    }else{ //unit already in english
      return print_unit(unit,flags);
    }
  } else {
    var unit = id;
