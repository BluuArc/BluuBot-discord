...
function print_unit(unit_id, flags, context){
    ...
    function print_burst_info(burst,name,field_arr,flags,burst_translation){
        try {
            var drop_checks = print_burst_dc_info(burst);
            let extra_data = (drop_checks === "None") ? "None" : (drop_checks + "\n" + burst.desc);
            if (flags.print_raw_effects && drop_checks !== "None") {
                extra_data = print_burst_effects(burst);
    ...
    function print_embed(unit,flags){
        return new Promise(function(fulfill,reject){
            ...
            let buff_translation = {
                ls: ep.printLS(),
                bb: ep.printBurst("bb"),
                sbb: ep.printBurst("sbb"),
                ubb: ep.printBurst("ubb"),
                es: ep.printES(),
                sp: ep.printSP()
            };
            ...
            if(flags.print_art){
                ...
                common.doesLinkExist(`${server}${files[0].filename}${unit.id}_2.png`)
                    .then(function(result){
                        ...
                        embeds.push("Issues viewing these? You can try again by adding `--noembed` to the end of your command");
            ...
            if(flags.print_sp){
                ...
                for(var b = 0; b < bursts.length; ++b){
                    ...
                    if(burst){
                        ...
                        let id = burst["damage frames"][0]["proc id"] || burst["damage frames"][0]["unknown proc id"];
                        ...
                        if (attacking_procs.indexOf(id) > -1 || attacking_unknown_procs.indexOf(id) > -1) {
                            ...
                            try {
                                for (var i = 1; i < burst["damage frames"].length; ++i) {
                                    id = burst["damage frames"][i]["proc id"] || burst["damage frames"][i]["unknown proc id"];
                                    if (attacking_procs.indexOf(id) > -1 || attacking_unknown_procs.indexOf(id) > -1) {
                ...
                if(unit["extra skill"]){
                    try{
                        ...
                        let local_field_arr = common.createFieldsArray(`Extra Skill: ${es.name}`, "", [extra_data, `\n**Buff Translation:**\n${buff_translation.es}`]);
                        for (let f of local_field_arr) {
                            field_arr.push(f);
                        }
                    } catch (err) {
                ...
                if(unit.bb){
                    print_burst_info(unit.bb,"Brave Burst",field_arr,flags,buff_translation.bb);
                }

                if (unit.sbb) {
                    print_burst_info(unit.sbb,"Super Brave Burst",field_arr,flags,buff_translation.sbb);
                }

                if (unit.ubb) {
                    print_burst_info(unit.ubb,"Ultimate Brave Burst", field_arr,flags,buff_translation.ubb);
                }
            }
            ...
            if (common.isJapaneseText(JSON.stringify(embed))) {
                notes.push("* Looks like this has some Japanese text in it. If this isn't what you wanted, try running the command again with `--translate`\n");
            }

            notes.push("* Please note that buff translations are still a work in progress and as of June 26, 2017, passive buffs and some proc buffs aren't supported yet\n");

            let notes_msg = common.create_sectional_messages_regular(notes, 900);
            for (let n = 0; n < notes_msg.length; ++n) {
                embed.fields.push({
                    name: `Notes - ${n + 1}`,
                    value: notes_msg[n]
                });
