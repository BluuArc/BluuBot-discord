        ...
        try{
            return new Promise(function(fulfill,reject){
                ...
                var process_results = search_results.then(function (results) {
                    ...
                    }else if(results.length >= 2000 && argv.flags.print_procs){
                        return print_procs();
                    } else {
...
function print_multiple_results(results, context) {
    function print_embed(result_arr, prefix) {
        ...
        embed.fields.push({
            name: "Search Help",
            value: "Not sure how to search? Try `--search_help` to see more filter options. Note that the number of results is limited to a max of 2000 for a faster search experience."
        });
    ...
    function print_regular(result_arr, prefix) {
        ...
        if (result_arr.length > max) {
            msg += "...and " + (result_arr.length - max) + " more. Note that the number of results is limited to a max of 2000 for a faster search experience.";
        }
