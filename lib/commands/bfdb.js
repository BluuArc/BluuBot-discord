...
var command_flags = [
    ...
    {
        name: 'limit',
        desc: 'specifies how many entries to print at most; use carefully with certain entries; must be a positive number',
        type: 'number',
        internal_type: 'print',
        default: 250
    }
    ...
    fn: (argv, context) => {
        return new Promise(function(fulfill,reject){
            ...
            if (argv.flags.reload_db !== '*') {
                ...
                Promise.resolve(reload_database(argv.flags.reload_db,context,argv.flags.server))
                    .then((result) => {
                        fulfill(result);
                    })
                return;
            ...
            if (argv.flags.server.length > 1 && argv.flags.server.indexOf("all") === -1)
                servers = argv.flags.server.toLowerCase().split(",").filter((s) => { return valid_servers.indexOf(s) > -1; });
            else
            ...
            if(!singlePrint && argv.flags.server === '*'){
                ...
                let header = "To see more info about each update, you can use any combination of the following commands with `|bb bfdb`: ";
                let footer = "\nTo print to full list for any entry, you must specify both a single server and print type. For example, `|bb bfdb --server jp --p_items` would print the full list of new items for the JP server, while `|bb bfdb --server all` would print the first 5 new entries for every server and print type.";
                fulfill(common.print_help(header,'print',footer,command_flags));
                return;
            }
            ...
            client.getStatus()
                .then(function(result){
                    ...
                    for(let s of servers){
                        if(singlePrint){ //print a single type
                            let lowerCaseTypes = types.map((t) => { return t.toLowerCase(); });
                            // console.log(singlePrint,lowerCaseTypes);
                            promises.push(get_server_statistics(result[s], mapping[s], types[lowerCaseTypes.indexOf(singlePrint)],singlePrint,servers.length,argv.flags.limit));
                        }else{ //print all types
                            for(var t = 0; t < types.length; ++t){
                                promises.push(get_server_statistics(result[s],mapping[s],types[t],singlePrint,servers.length,argv.flags.limit));
                            }
                        }
                    }
...
function get_server_statistics(server_stats, server_name, type, doSinglePrint, totalServers, limit){
    ...
    return new Promise(function(fulfill,reject){
        ...
        var parsed_newest;
        let max, hardLimit = (limit > 0 ? limit : 250);
        if (newest.length <= 5) {
            max = newest.length;
        } else if (!doSinglePrint && totalServers !== 1) { //print first 5 if not doing a specific print
            max = 5;
            msg += "The first 5 are:";
        } else if(!doSinglePrint || totalServers !== 1){ //print first 10 if doing partially specifig print
            if(newest.length <= 10){
                max = newest.length;
            }else{
                max = 10;
        ...
        }else if(newest.length > hardLimit){ //cap output to given limit
            max = hardLimit;
            msg += `The first ${hardLimit} are:`;
        }else{
            max = newest.length;
        }
...
function reload_database(input,context,server) {
    ...
    if (pass === input) {
        ...
        let valid_servers = ['gl', 'eu', 'jp'];
        let servers;
        if (server.length > 1 && server.indexOf("all") === -1)
            servers = server.toLowerCase().split(",").filter((s) => { return valid_servers.indexOf(s) > -1; });
        else
            servers = valid_servers;

        if (server.length < 2 || servers.length === 0){
            return(`Error: No valid server(s) entered. Please enter a valid server with \`--server ${valid_servers.join(" or ")}\``);
        }

        var options = {
            method: 'GET',
            uri: url + "/reload?servers=" + servers.join(",")
        };
