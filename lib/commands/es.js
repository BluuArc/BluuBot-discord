...
module.exports = new MyCommand({
    name: "es",
    desc: "returns an ES/Elgif based on input; use es --search_help and es --print_help for more info; only workds for non-unit extra skills",
    args: command_args,
    flags: command_flags,
    fn: (argv, context) => {
        try {
...
function print_es(es_id,flags,context){
    function print_embed(es,flags){
        ...
        return new Promise(function(fulfill,reject){
            ...
            if(undefined){ //placeholder for any future features
                ...
                let buff_translation = ep.printES(es).split(" / ").map((s) => { return `* ${s}\n`; }).join("");
    ...
    function print_regular(es,flags){
        return print_embed(es,flags)
            .then((embed) => {
                return common.convertEmbedToText(embed.embed);
            });
    }

    function print_regular_old(es,flags){
        let es_name = (es.translated_name) ? (`${es.translated_name} (${es.id})`) : (`${es.name} (${es.id})`);
