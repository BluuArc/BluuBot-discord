...
function print_item(item_id, flags, context){
  ...
  function print_embed(item,flags){
    ...
    return new Promise(function(fulfill,reject){
      ...
      //general info
      embed.description = `_Type:_ ${item.type}`;
      if(item["sphere type text"]){
        embed.description += ` (${item["sphere type text"]})`;
      }
      embed.description += `\n_Sell Price:_ ${print_formatted_number(item.sell_price)} Zel`;
      if(item["max equipped"]){
        embed.description += `\n_Max Equipped:_ ${item["max equipped"]}`;
      }
      embed.description += `\n_Max Stack:_ ${item.max_stack}`;
