...
function common_library(){
    ...
    function print_effects(effects) {
        var text_arr = [];
        const ignored_fields = ['passive id', 'proc id', 'ffect delay time(ms)\/frame']
        //convert each effect into its own string
        for (var param in effects) {
            if (ignored_fields.indexOf(param) === -1) {
    ...
    function convertEmbedToText(embed){
        let msg = [];
        if(embed.title){
            msg.push(`***${embed.title}***\n`);
        }

        if(embed.description){
            msg.push(`${embed.description}\n`);
        }

        if(embed.fields){
            for(let f = 0; f < embed.fields.length; ++f){
                let curMsg = "";
                let curField = embed.fields[f];
                if(curField.name){
                    curMsg += `**${curField.name}**\n`;
                }else{
                    curMsg += `**Field ${f+1}:**\n`;
                }
                ...
                if(curField.value){
                    curMsg += `${curField.value}\n`;
                }else{
                    curMsg += `None\n`;
                }

                //add extra new line if one doesn't exist already
                if(!msg.slice(-1)[0].endsWith("\n\n")){
                    msg.push("\n" + curMsg);
                }else{
                    msg.push(curMsg);
                }
        ...
        return create_sectional_messages_regular(msg,1900);
    }
    this.convertEmbedToText = convertEmbedToText;

    //prints available commands
    function print_flag_options(command_flags) {
        let command_arr = ['Available Command Flags (meant to be placed at the end of your current command):\n'];
        for (let c of command_flags) {
            command_arr.push(`* \`--${c.name}\`: ${c.desc}\n`);
        }
        return JSON.stringify(create_sectional_messages_regular(command_arr, 1750));
    }
    this.print_flag_options = print_flag_options;
}
